...
fn map_return_type(output: &syn::ReturnType) -> TokenStream {
    match output {
        syn::ReturnType::Default => {
            ...
            quote! { ndc_interpreter::interpreter::function::StaticType::Tuple(vec![]) }
        }
...
fn map_type(ty: &syn::Type) -> TokenStream {
    match ty {
        ...
        syn::Type::Tuple(t) => {
            ...
            quote::quote! {
                ndc_interpreter::interpreter::function::StaticType::Tuple(vec![
                    #(#inner),*
                ])
            }
        }
        syn::Type::Infer(_) => {
            quote::quote! { ndc_interpreter::interpreter::function::StaticType::Any }
        }
        _ => {
...
fn map_type_path(p: &syn::TypePath) -> TokenStream {
    ...
    match segment.ident.to_string().as_str() {
        "i32" | "i64" | "isize" | "u32" | "u64" | "usize" | "BigInt" => {
            quote::quote! { ndc_interpreter::interpreter::function::StaticType::Int }
        }
        "f32" | "f64" => {
            quote::quote! { ndc_interpreter::interpreter::function::StaticType::Float }
        }
        "bool" => {
            quote::quote! { ndc_interpreter::interpreter::function::StaticType::Bool }
        }
        "String" | "str" => {
            quote::quote! { ndc_interpreter::interpreter::function::StaticType::String }
        }
        "Vec" | "List" => match &segment.arguments {
            syn::PathArguments::AngleBracketed(args) => {
                ...
                if let syn::GenericArgument::Type(inner_ty) = inner {
                    ...
                    quote::quote! { ndc_interpreter::interpreter::function::StaticType::List(Box::new(#mapped)) }
                } else {
                    panic!("Vec inner not a type");
                }
            }
            _ => {
                quote::quote! { ndc_interpreter::interpreter::function::StaticType::List(Box::new(ndc_interpreter::interpreter::function::StaticType::Any)) }
            }
        },
        "VecDeque" | "Deque" => match &segment.arguments {
            syn::PathArguments::AngleBracketed(args) => {
                ...
                if let syn::GenericArgument::Type(inner_ty) = inner {
                    let mapped = map_type(inner_ty);
                    quote::quote! { ndc_interpreter::interpreter::function::StaticType::Deque(Box::new(#mapped)) }
                } else {
                    panic!("VecDeque inner not a type");
                }
            }
            _ => quote::quote! {
                ndc_interpreter::interpreter::function::StaticType::Deque(Box::new(
        ...
        "DefaultMap" | "HashMap" | "Map" => match &segment.arguments {
            syn::PathArguments::AngleBracketed(args) => {
                ...
                quote::quote! { ndc_interpreter::interpreter::function::StaticType::Map { key: Box::new(#key_mapped), value: Box::new(#val_mapped) } }
            }
        ...
        "MinHeap" => match &segment.arguments {
            syn::PathArguments::AngleBracketed(args) => {
                ...
                if let syn::GenericArgument::Type(inner_ty) = inner {
                    let mapped = map_type(inner_ty);
                    quote::quote! { ndc_interpreter::interpreter::function::StaticType::MinHeap(Box::new(#mapped)) }
                } else {
                    panic!("MinHeap inner invalid");
                }
            }
            _ => quote::quote! {
                ndc_interpreter::interpreter::function::StaticType::MinHeap(Box::new(
                    ndc_interpreter::interpreter::function::StaticType::Any
                ))
            },
        },
        "MaxHeap" => match &segment.arguments {
            syn::PathArguments::AngleBracketed(args) => {
                ...
                if let syn::GenericArgument::Type(inner_ty) = inner {
                    let mapped = map_type(inner_ty);
                    quote::quote! { ndc_interpreter::interpreter::function::StaticType::MaxHeap(Box::new(#mapped)) }
                } else {
        ...
        "Iterator" => match &segment.arguments {
            syn::PathArguments::AngleBracketed(args) => {
                ...
                if let syn::GenericArgument::Type(inner_ty) = inner {
                    let mapped = map_type(inner_ty);
                    quote::quote! { ndc_interpreter::interpreter::function::StaticType::Iterator(Box::new(#mapped)) }
                } else {
                    panic!("Iterator inner invalid");
                }
            }
            _ => {
                quote::quote! { ndc_interpreter::interpreter::function::StaticType::Iterator(Box::new(ndc_interpreter::interpreter::function::StaticType::Any)) }
            }
        },
        "Option" => match &segment.arguments {
            syn::PathArguments::AngleBracketed(args) => {
                ...
                if let syn::GenericArgument::Type(inner_ty) = inner {
                    let mapped = map_type(inner_ty);
                    quote::quote! { ndc_interpreter::interpreter::function::StaticType::Option(Box::new(#mapped)) }
                } else {
        ...
        "Number" => quote::quote! { ndc_interpreter::interpreter::function::StaticType::Number },
...
fn wrap_single(
    ...
    let return_expr = match function.sig.output {
        syn::ReturnType::Default => quote! {
            return Ok(ndc_interpreter::interpreter::value::Value::unit());
        },
        syn::ReturnType::Type(_, typ) => match &*typ {
            ...
            ty @ syn::Type::Path(_) if path_ends_with(ty, "Result") => quote! {
                let value = result.map_err(|err| ndc_interpreter::interpreter::function::FunctionCarrier::IntoEvaluationError(Box::new(err)))?;
                return Ok(ndc_interpreter::interpreter::value::Value::from(value));
            },
            _ => quote! {
                let result = ndc_interpreter::interpreter::value::Value::from(result);
                return Ok(result);
            },
        },
    };
    ...
    let function_declaration = quote! {
        pub fn #identifier (
            values: &mut [ndc_interpreter::interpreter::value::Value],
            environment: &std::rc::Rc<std::cell::RefCell<ndc_interpreter::interpreter::environment::Environment>>
        ) -> ndc_interpreter::interpreter::evaluate::EvaluationResult {
    ...
    let function_registration = quote! {
        let func = ndc_interpreter::interpreter::function::FunctionBuilder::default()
            .body(ndc_interpreter::interpreter::function::FunctionBody::GenericFunction {
                function: #identifier,
                type_signature: ndc_interpreter::interpreter::function::TypeSignature::Exact(vec![
                    #( ndc_interpreter::interpreter::function::Parameter::new(#param_names, #param_types,) ),*
                ]),
...
fn into_param_type(ty: &syn::Type) -> TokenStream {
    match ty {
        ty if path_ends_with(ty, "Vec") => {
            quote! { ndc_interpreter::interpreter::function::StaticType::List(Box::new(ndc_interpreter::interpreter::function::StaticType::Any)) }
        }
        ty if path_ends_with(ty, "VecDeque") => {
            quote! { ndc_interpreter::interpreter::function::StaticType::Deque(Box::new(ndc_interpreter::interpreter::function::StaticType::Any)) }
        }
        ...
        ty if path_ends_with(ty, "MinHeap") => {
            quote! { ndc_interpreter::interpreter::function::StaticType::MinHeap(Box::new(ndc_interpreter::interpreter::function::StaticType::Any)) }
        }
        ty if path_ends_with(ty, "MaxHeap") => {
            quote! { ndc_interpreter::interpreter::function::StaticType::MaxHeap(Box::new(ndc_interpreter::interpreter::function::StaticType::Any)) }
        }
        ...
        syn::Type::Path(syn::TypePath { path, .. }) => match path {
            _ if path.is_ident("i64") => quote! { ndc_interpreter::interpreter::function::StaticType::Int },
            _ if path.is_ident("usize") => {
                quote! { ndc_interpreter::interpreter::function::StaticType::Int }
            }
            _ if path.is_ident("f64") => {
                quote! { ndc_interpreter::interpreter::function::StaticType::Float }
            }
            _ if path.is_ident("bool") => {
                quote! { ndc_interpreter::interpreter::function::StaticType::Bool }
            }
            _ if path.is_ident("Value") => {
                quote! { ndc_interpreter::interpreter::function::StaticType::Any }
            }
            _ if path.is_ident("Number") => {
                quote! { ndc_interpreter::interpreter::function::StaticType::Number }
            }
            _ if path.is_ident("Sequence") => {
                quote! { ndc_interpreter::interpreter::function::StaticType::Sequence(Box::new(ndc_interpreter::interpreter::function::StaticType::Any)) }
            }
            _ if path.is_ident("Callable") => {
                quote! {
                    ndc_interpreter::interpreter::function::StaticType::Function {
                        parameters: None,
                        return_type: Box::new(ndc_interpreter::interpreter::function::StaticType::Any)
                    }
                }
            }
        ...
        syn::Type::ImplTrait(_) => {
            quote! { ndc_interpreter::interpreter::function::StaticType::Iterator(Box::new(ndc_interpreter::interpreter::function::StaticType::Any)) }
        }
    ...
    if let syn::FnArg::Typed(pat_type) = input {
        ...
        if path_ends_with(ty, "Callable") {
            ...
            return vec![Argument {
                ...
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Function(#temp_var) = #argument_var_name else {
        ...
        // The pattern is &HashMap<Value, Value>
            ...
            return vec![Argument {
                ...
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Sequence(ndc_interpreter::interpreter::sequence::Sequence::Map(#rc_temp_var, _default)) = #argument_var_name else {
                        panic!("Value #position needed to be a Sequence::Map but wasn't");
                    };
        ...
        // The pattern is DefaultMap
            ...
            return vec![Argument {
                ...
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Sequence(ndc_interpreter::interpreter::sequence::Sequence::Map(#rc_temp_var, default)) = #argument_var_name else {
                        panic!("Value #position needed to be a Sequence::Map but wasn't");
                    };
        ...
        // TODO: support this for tuple
            ...
            let rc_temp_var =
                syn::Ident::new(&format!("temp_{argument_var_name}"), identifier.span());
            return vec![Argument {
                param_type: quote! { ndc_interpreter::interpreter::function::StaticType::List(Box::new(ndc_interpreter::interpreter::function::StaticType::Any)) },
                param_name: quote! { #original_name },
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Sequence(ndc_interpreter::interpreter::sequence::Sequence::List(#rc_temp_var)) = #argument_var_name else {
        ...
        // The pattern is exactly &mut VecDeque
            ...
            return vec![Argument {
                ...
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Sequence(ndc_interpreter::interpreter::sequence::Sequence::Deque(#rc_temp_var)) = #argument_var_name else {
                        panic!("Value #position needed to be a Sequence::List but wasn't");
                    };
                    let #argument_var_name = &mut *#rc_temp_var.try_borrow_mut()?;
                },
        ...
        // The pattern is exactly &mut MaxHeap
            ...
            return vec![Argument {
                ...
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Sequence(ndc_interpreter::interpreter::sequence::Sequence::MaxHeap(#rc_temp_var)) = #argument_var_name else {
        ...
        // The pattern is exactly &mut MinHeap
            ...
            return vec![Argument {
                ...
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Sequence(ndc_interpreter::interpreter::sequence::Sequence::MinHeap(#rc_temp_var)) = #argument_var_name else {
        ...
        // The pattern is exactly &str
            ...
            let rc_temp_var =
                syn::Ident::new(&format!("temp_{argument_var_name}"), identifier.span());
            return vec![Argument {
                param_type: quote! { ndc_interpreter::interpreter::function::StaticType::String },
                param_name: quote! { #original_name },
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Sequence(ndc_interpreter::interpreter::sequence::Sequence::String(#rc_temp_var)) = #argument_var_name else {
        ...
        // The pattern is &BigInt
            ...
            return vec![Argument {
                param_type: quote! { ndc_interpreter::interpreter::function::StaticType::Int },
                param_name: quote! { #original_name },
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let #big_int = if let ndc_interpreter::interpreter::value::Value::Number(ndc_interpreter::interpreter::num::Number::Int(ndc_interpreter::interpreter::int::Int::Int64(smol))) = #argument_var_name {
                    ...
                    let #argument_var_name = match #argument_var_name {
                        ndc_interpreter::interpreter::value::Value::Number(ndc_interpreter::interpreter::num::Number::Int(ndc_interpreter::interpreter::int::Int::BigInt(big))) => big,
                        ndc_interpreter::interpreter::value::Value::Number(ndc_interpreter::interpreter::num::Number::Int(ndc_interpreter::interpreter::int::Int::Int64(smoll))) => #big_int.as_ref().unwrap(),
        ...
        // If we need an owned Value
            ...
            return vec![Argument {
                param_type: quote! { ndc_interpreter::interpreter::function::StaticType::Any },
                param_name: quote! { #original_name },
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
        ...
        // The pattern is &[Value]
            ...
            return vec![
                ...
                //     argument: quote! { #argument_var_name },
                //     initialize_code: quote! {
                //         let ndc_interpreter::interpreter::value::Value::Sequence(ndc_interpreter::interpreter::sequence::Sequence::Tuple(#rc_temp_var)) = #argument_var_name else {
                //             panic!("Value #position needed to be a Sequence::List but wasn't");
                //         };
        ...
        // The pattern is &BigRational
            ...
            return vec![Argument {
                param_type: quote! { ndc_interpreter::interpreter::function::StaticType::Rational },
                param_name: quote! { #original_name },
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Number(ndc_interpreter::interpreter::num::Number::Rational(#argument_var_name)) = #argument_var_name else {
        ...
        // The pattern is Complex64
            ...
            return vec![Argument {
                param_type: quote! { ndc_interpreter::interpreter::function::StaticType::Complex },
                param_name: quote! { #original_name },
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Number(ndc_interpreter::interpreter::num::Number::Complex(#argument_var_name)) = #argument_var_name else {
        ...
        // The pattern is something like `i64` (but also matches other concrete types)
            ...
            return vec![Argument {
                ...
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let #argument_var_name = #path :: try_from(#argument_var_name).map_err(|err| ndc_interpreter::interpreter::function::FunctionCallError::ConvertToNativeTypeError(format!("{err}")))?
                },
            }];
        }
        // The pattern is something like '&Number'
            ...
            return vec![Argument {
                ...
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let #argument_var_name = <#type_ref as TryFrom<&mut ndc_interpreter::interpreter::value::Value>> :: try_from(#argument_var_name).map_err(|err| ndc_interpreter::interpreter::function::FunctionCallError::ConvertToNativeTypeError(format!("{err}")))?
                },
            }];
        }
        // The pattern is a trait implementation TODO: this is not implemented
            ...
            return vec![Argument {
                ...
                argument: quote! { #argument_var_name },
                initialize_code: quote! {
                    let ndc_interpreter::interpreter::value::Value::Sequence(ndc_interpreter::interpreter::sequence::Sequence::Iterator(#rc_temp_var)) = #argument_var_name else {
                        panic!("Value #position needed to be a Sequence::Iterator but wasn't");
                    };

                    let #argument_var_name = ndc_interpreter::interpreter::iterator::RcIter::new(Rc::clone(#rc_temp_var));
                },
            }];
        } else {
...
pub fn temp_create_map_any() -> TokenStream {
    quote! {
       ndc_interpreter::interpreter::function::StaticType::Map {
           key: Box::new(ndc_interpreter::interpreter::function::StaticType::Any),
           value: Box::new(ndc_interpreter::interpreter::function::StaticType::Any)
       }
    }
}
