...
impl Parser {
    ...
    fn peek_type_arguments(&self) -> bool {
        ...
        for location in self.tokens.iter().skip(self.current + 1) {
            // The number of `>` this token contributes, and whether it leaves a
            // remainder (the `=` of `>=` and `>>=`) that cannot occur in a type.
            let (closers, closes_cleanly) = match &location.token {
                ...
                | Token::Comma
                | Token::LeftParentheses
                | Token::RightParentheses => continue,
