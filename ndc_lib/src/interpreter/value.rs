...
impl Value {
    ...
    pub fn static_type(&self) -> StaticType {
        match self {
            ...
            Self::Sequence(Sequence::MinHeap(_)) => StaticType::MinHeap,
            Self::Sequence(Sequence::Deque(_)) => StaticType::Deque,
        }
    }

    #[must_use]
    pub fn empty_list() -> Self {
        Self::Sequence(Sequence::List(Rc::new(RefCell::new(vec![]))))
    }

    #[must_use]
    pub fn deepcopy(&self) -> Self {
        match self {
            Self::Sequence(seq) => Self::Sequence(seq.deepcopy()),
...
impl PartialOrd for Value {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match (self, other) {
            (Self::Option(left), Self::Option(right)) => left.partial_cmp(right),
            (Self::Number(left), Self::Number(right)) => left.partial_cmp(right),
            (Self::Sequence(left), Self::Sequence(right)) => left.partial_cmp(right),
...
impl From<usize> for Value {
    fn from(value: usize) -> Self {
        i64::try_from(value).map_or_else(|_| Self::from(BigInt::from(value)), Self::from)
    }
}
...
impl From<RangeFrom<i64>> for Value {
    fn from(value: RangeFrom<i64>) -> Self {
        Self::from(ValueIterator::ValueRangeFrom(ValueRangeFrom(value)))
    }
}

impl From<Range<i64>> for Value {
    fn from(value: Range<i64>) -> Self {
        Self::from(ValueIterator::ValueRange(ValueRange(value)))
    }
}
